...
// If the containers are used, the Widgets should always referred to with Smart_Pointers.

class Widget : public Reference_Counted_Object {
public:
	...
	Smart_Pointer<Container_widget> get_container() const;
	...
	void set_container(Smart_Pointer<Container_widget> container_ptr_);
...
private:
	...
	Smart_Pointer<Container_widget> container_ptr;	// pointer to a container widget, if any
...
struct Widget_Sptr_widget_name_less {
	bool operator() (const Smart_Pointer<Widget> wp1, const Smart_Pointer<Widget> wp2) const
...
private:
	Smart_Pointer<Label_widget> label_ptr;
	Smart_Pointer<Field_widget> field_ptr;
};
...
public:
	...
	void add_widget(Smart_Pointer<Widget>);
	// remove this widget from the display and the container
	void remove_widget(Smart_Pointer<Widget>);
	// new location is relative to this window, and so is adjusted
	void set_relative_location(Smart_Pointer<Widget>, GU::Point);
	
	Smart_Pointer<Widget> get_widget_ptr(const Symbol& widget_name) const;
	
	virtual void remove(Smart_Pointer<Widget> widget_ptr);

	virtual void present_object();
	virtual void depresent();

private:
	typedef std::map<Symbol, Smart_Pointer<Widget> > Widget_Sptrs_t;
