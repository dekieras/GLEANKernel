...
Widget::Widget(Device_base * device_ptr_, const Symbol& widget_name_, GU::Point location_, GU::Size size_) : 
	device_ptr(device_ptr_), widget_name(widget_name_), location(location_), size(size_), presented(NEVER_PRESENTED), container_ptr(0)
{
}
...
Smart_Pointer<Container_widget> Widget::get_container() const
{
	return container_ptr;
}

void Widget::set_container(Smart_Pointer<Container_widget> container_ptr_)
{
...
Polygon_widget::Polygon_widget(Device_base * device_ptr_, const Symbol& widget_name_, 
	...
	Object_widget(device_ptr_, widget_name_, polygon.get_center(), polygon.get_size())
{
...
Line_widget::Line_widget(Device_base * device_ptr_, const Symbol& widget_name_, 
	...
	Object_widget(device_ptr_, widget_name_, line.get_center(), line.get_size())
{
	set_property(Color_c, color_);
			...
			))
{
	...
	label_ptr = new Label_widget(device_ptr, label_name, location,  label_size, label, color);
	field_ptr = new Field_widget(device_ptr, value_name, 
...
void Container_widget::add_widget(Smart_Pointer<Widget> widget_ptr)
{
...
void Container_widget::remove_widget(Smart_Pointer<Widget> widget_ptr)
{
	Assert(widgets.find(widget_ptr->get_name()) != widgets.end());
...
void Container_widget::set_location(GU::Point new_location)
{
	...
	for(Widget_Sptrs_t::iterator it = widgets.begin(); it != widgets.end(); ++it) {
		Smart_Pointer<Widget> widget_ptr = it->second;
...
void Container_widget::set_relative_location(Smart_Pointer<Widget> widget_ptr, GU::Point new_location)
{
	GU::Point cp = get_location();
...
Smart_Pointer<Widget> Container_widget::get_widget_ptr(const Symbol& widget_name) const
{
...
void Container_widget::remove(Smart_Pointer<Widget> widget_ptr)
{
	widget_ptr->depresent();
	widgets.erase(widget_ptr->get_name());
}
		...
		const Symbol& title) : Container_widget(device_ptr, widget_name, location, size)
{
	...
	Smart_Pointer<Widget> ptr = new Label_widget(device_ptr, Symbol(title_widget_name_s), title_loc, title_size, title);
	add_widget(ptr);
}
