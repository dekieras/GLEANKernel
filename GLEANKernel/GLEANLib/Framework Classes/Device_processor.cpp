...
Device_processor::Device_processor(const std::string& id) :
	Processor(id), device_interface_ptr(0), human_ptr(0)
{
}
...
void Device_processor::make_auditory_speech_sound_event(const Symbol& name, const Symbol& source, const Symbol& utterance, long duration)
{
	if(Trace_out && get_trace()) 
		Trace_out << processor_info() << " Speech sound event: " 
			<< name << ' ' << source << ' ' << utterance << ' ' << duration << endl;	
	human_interface_ptr->make_auditory_speech_sound_event(name, source, utterance, duration);
}
